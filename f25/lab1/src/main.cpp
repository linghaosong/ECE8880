...
int main(int argc, char** argv) {
  ...
  //host data
  aligned_vector<float> v1(kVectorLen);
  aligned_vector<float> v2(kVectorLen);
  aligned_vector<float> v_result_dev(kVectorLen, 0.0);
  aligned_vector<float> v_result_host(kVectorLen, 0.0);

  InitializeData(v1, v2);

  Vadd_host(v1, v2, v_result_host);
  ...
  //verify
  int error = Verify(v_result_dev, v_result_host);
  if (error != 0) {
